import { MimeTypesMap } from './FileUploader.types';

// https://developer.mozilla.org/en-US/docs/Web/HTTP/Basics_of_HTTP/MIME_types/Common_types
export const mimeTypesMap: MimeTypesMap = {
  image: {
    bmp: 'BMP',
    gif: 'GIF',
...
export const convertMimeTypeToFileType = (mimeType: string): string => {
  ...
  if (!fileType) fileType = mimeTypeEnd.toUpperCase();
...
export const formatMimeTypesForDisplay = (mimeTypes: string[]): string =>
  mimeTypes.reduce((acc, curr, index, inputArray) => {
    if (index !== 0) acc += inputArray.length > 2 ? ', ' : ' ';

    if (index === inputArray.length - 1 && inputArray.length > 1) acc += 'or ';

    acc += convertMimeTypeToFileType(curr);

    if (inputArray.length === 1) acc += ' only';
